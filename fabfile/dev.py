...
def clear(app_name, model_name):
    ...
    """
    local("echo 'DROP TABLE {0}_{1};' | psql homelessness".format(
        app_name, model_name
        )
    )
...
def destroy():
    ...
    answer = raw_input("> ")
    if (answer == 'homelessness'):
        dropdb()
        local('cd .. && rm -rf {0}'.format(project_name))
        log("homelessness is no more. See you later!\n", "green")

    else:
        log("You didn't type 'homelessness' correctly. Exiting.")
